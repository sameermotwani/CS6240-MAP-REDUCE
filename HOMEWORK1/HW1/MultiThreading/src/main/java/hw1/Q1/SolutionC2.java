...
import java.util.Map;
 ...
 class SolutionC2Thread implements Runnable
{

	private List<String> Input;
	private Map<String,Float[]> allTmaxRecords;
	
	public SolutionC2Thread(List<String> Input, Map<String,Float[]> TotalTmaxPerStation){
		this.Input=Input;
...
public class SolutionC2
{
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		for (int i = 0; i < 10; i++) {
			...
			Thread t1=new Thread(new SolutionC2Thread(Input1,allTmaxRecords));
			Thread t2=new Thread(new SolutionC2Thread(Input2,allTmaxRecords));
			Thread t3=new Thread(new SolutionC2Thread(Input3,allTmaxRecords));
			Thread t4=new Thread(new SolutionC2Thread(Input4,allTmaxRecords));
		...
		File op=new File("output/C2Output.csv");
